...
class DiffSet(models.Model):
    ...
    def incremenent_commit_comment_counts(self, commits_to_incr_comment_counts):
        for c in commits_to_incr_comment_counts:
            for diff_commit in self.commits.filter(pk=c):
                diff_commit.increment_num_comments()
