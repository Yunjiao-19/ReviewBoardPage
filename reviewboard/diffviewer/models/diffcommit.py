...
class DiffCommit(models.Model):
    ...
    def increment_num_comments(self):
        old_num_comments = self.extra_data.get('num_comments')

        if old_num_comments is None:
            old_num_comments = 0
        
        self.extra_data.update({
            'num_comments': old_num_comments + 1
        })

        self.save()

    def get_num_comments(self):
        num_comments = self.extra_data.get('num_comments')
