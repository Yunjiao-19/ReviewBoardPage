...
class DiffViewerView(TemplateView):
    ...
    def get_context_data(self, diffset, interdiffset, extra_context={},
        ...
        # Team 17
        # Get current revision's all reviewer's read status
        ...
        for i in range(len(user_name_id)):
            ...
            for obj in fileset.values():
                ...
                if obj.status == 1:
                    tentative += 1 
            status_count[user_name_lst[i]] = [0,0]
            status_count[user_name_lst[i]][0] = read
            status_count[user_name_lst[i]][1] = tentative
        
        last_diffset = review_request.get_latest_diffset()
        last_files = get_diff_files(diffset=last_diffset, request=ReviewRequest.objects.filter(id=review_request_id)[0])
        last_files_set = set(f['dest_filename'] for f in last_files)
        total = len(last_files_set)
        

        context = dict({
            # Team 17
            # Pass current reviewer's name and all reviewer's read status to frontend
            'submitter_name': self.request.user.username,
            'read_status_count': status_count,
            'total_files': total,
            'reviewer_names': user_name_lst,
