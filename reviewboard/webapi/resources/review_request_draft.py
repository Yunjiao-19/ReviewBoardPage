...
from reviewboard.reviews.models import Group, ReviewRequest, ReviewRequestDraft, ReadStatus
...
class ReviewRequestDraftResource(MarkdownFieldsMixin, WebAPIResource):
    ...
    def update(self,
        ...
        if request.POST.get('public', False):
            ...
            last_diffset = review_request.get_latest_diffset() 
            last_files = get_diff_files(diffset=last_diffset, request=request) \
                if last_diffset else []
            last_files_set = set(f['dest_filename'] for f in last_files)

            reviewers = User.objects.filter(directed_drafts=draft)
            if not reviewers:
                reviewers = User.objects.filter(\
                    directed_review_requests=review_request)
            last_revision = last_diffset.revision
            if last_revision == 1: # initialize for the first revision
                for file_name in last_files_set:
                    for reviewer in reviewers:
                        # create entry if not already initialized
                        try:
                            s = ReadStatus.objects.filter(\
                                user_id=reviewer.id, \
                                file_name=file_name,\
                                review_request_id=review_request.display_id, \
                                revision_id=last_revision).latest('timestamp').status
                        except:
                            ReadStatus.objects.create(\
                                user_id=reviewer.id, \
                                file_name=file_name,\
                                status=0, # status initialized to be 0
                                review_request_id=review_request.display_id, \
                                revision_id=last_revision)
            elif last_revision > 1: # change status based on previous revision
                # get second to last revision diffset
                prev_diffset = list(DiffSet.objects.filter(\
                                history=review_request.diffset_history_id))[-2]
                prev_files = get_diff_files(\
                                diffset=prev_diffset, request=request) \
                                if prev_diffset else []
                prev_files_set = set(f['dest_filename'] for f in prev_files)
                changed_files_set = get_diff_files(\
                                    last_diffset, interdiffset=prev_diffset)
                changed_files = [f['dest_filename'] for f in changed_files_set]

                for file_name in last_files_set:
                    for reviewer in reviewers:
                        if file_name not in prev_files_set or file_name in changed_files: 
                            # this is a new file to the revision or it is modified
                            ReadStatus.objects.create(user_id=reviewer.id, \
                                                    status=0, \
                                                    review_request_id=review_request.display_id,\
                                                    file_name=file_name,\
                                                    revision_id=last_revision)
                        else:
                            # this file persisted from previous revision
                            try:
                                prev_status = ReadStatus.objects.filter(\
                                                user_id=reviewer.id, \
                                                file_name=file_name,\
                                                review_request_id=review_request.display_id, \
                                                revision_id=last_revision-1).latest('timestamp').status
                            except:
                                prev_status = 0

                            if prev_status == 0:
                                # if previously unread, then still unread
                                curr_status = 0
                            else:
                                # if previously tentative/read, then tentatively read
                                curr_status = 1
                            ReadStatus.objects.create(
                                    user_id=reviewer.id, \
                                    status=curr_status, \
