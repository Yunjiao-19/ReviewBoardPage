...
from reviewboard.webapi.resources.review_untouched_comment import \
    ReviewUntouchedCommentResource


class ReviewReplyUntouchedCommentResource(BaseUntouchedCommentResource):
    ...
    policy_id = 'review_reply_untouched_comment'
    model_parent_key = 'review'
    fields = dict({
        'reply_to': {
            'type': ResourceFieldType,
            'resource': ReviewUntouchedCommentResource,
            'description': 'The comment being replied to.',
        },
    }, **BaseUntouchedCommentResource.fields)

    mimetype_list_resource_name = 'review-reply-untouched-comments'
    mimetype_item_resource_name = 'review-reply-untouched-comment'

    def get_queryset(self, request, review_id, reply_id, *args, **kwargs):
        q = super(ReviewReplyUntouchedCommentResource, self).get_queryset(
            request, *args, **kwargs)
    ...
    @webapi_response_errors(DOES_NOT_EXIST, INVALID_FORM_DATA,
                            NOT_LOGGED_IN, PERMISSION_DENIED)
    @webapi_request_fields(
        required=BaseUntouchedCommentResource.REPLY_REQUIRED_CREATE_FIELDS,
        optional=BaseUntouchedCommentResource.REPLY_OPTIONAL_CREATE_FIELDS,
        allow_unknown=True
    )
    def create(self, request, reply_to_id, *args, **kwargs):
        ...
        try:
            ...
            reply = Review.objects.get(id=kwargs['reply_id'])
        ...
        try:
            untouched_comment = resources.review_untouched_comment.get_object(
        ...
        return self.create_or_update_comment_reply(
            ...
            reply=reply,
            comments_m2m=reply.untouched_comments,
            default_attrs={
                # 'filediff': comment.filediff,
                # 'interfilediff': comment.interfilediff,
                'untouchedfile': untouched_comment.untouchedfile,
                'first_line': untouched_comment.first_line,
                'num_lines': untouched_comment.num_lines,
            },
            *args,
            **kwargs)
    ...
    @webapi_request_fields(
        optional=BaseUntouchedCommentResource.REPLY_OPTIONAL_UPDATE_FIELDS,
        allow_unknown=True
    )
...
review_reply_untouched_comment_resource = ReviewReplyUntouchedCommentResource()