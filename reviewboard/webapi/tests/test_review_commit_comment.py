...
from reviewboard.reviews.models import CommitMessageComment
from reviewboard.webapi.resources import resources
from reviewboard.webapi.tests.base import BaseWebAPITestCase
from reviewboard.webapi.tests.mimetypes import (
    commit_message_comment_item_mimetype,
    commit_message_comment_list_mimetype)
...
from reviewboard.webapi.tests.urls import (
    get_review_commit_comment_item_url,
    get_review_commit_comment_list_url)

class BaseTestCase(BaseWebAPITestCase):
    fixtures = ['test_users']

    # TODO: need to add commit_id to be passed in and create a commit message comment

    def _create_commit_comment_review_with_issue(self, publish=False,
                                          comment_text=None):
        """Sets up a review for a commit message comment that includes an open issue.
        ...
        """
        if not comment_text:
            comment_text = 'Test commit message comment with an opened issue'

        review_request = self.create_review_request(publish=True,
                                                    submitter=self.user)
        review = self.create_review(review_request, user=self.user,
                                    publish=publish)
        comment = self.create_commit_message_comment(review, comment_text,
                                              issue_opened=True)
...
class ResourceListTests(CommentListMixin, ReviewRequestChildListMixin,
                        BaseTestCase):
    """Testing the ReviewCommitMessageCommentResource list APIs."""
    sample_api_url = 'review-requests/<id>/reviews/<id>/commit-message-comments/'
    resource = resources.review_commit_comment

    def setup_review_request_child_test(self, review_request):
        review = self.create_review(review_request, user=self.user)

        return (get_review_commit_comment_list_url(review),
                commit_message_comment_list_mimetype)
    ...
    def setup_basic_get_test(self, user, with_local_site, local_site_name,
        ...
        if populate_items:
            items = [self.create_commit_message_comment(review)]
        else:
            items = []
    ...
    def check_post_result(self, user, rsp, review):
        ...
        comment = \
            CommitMessageComment.objects.get(pk=rsp['commit_message_comment']['id'])
        self.compare_item(rsp['commit_message_comment'], comment)

    def test_post_with_issue(self):
        """Testing the
        POST review-requests/<id>/reviews/<id>/commit-message-comments/ API
        with an issue
        """
        comment_text = "Test general comment with an opened issue"
        comment, review, review_request = \
            self._create_commit_comment_review_with_issue(
                publish=False, comment_text=comment_text)

        rsp = self.api_get(
            get_review_commit_comment_list_url(review),
            expected_mimetype=commit_message_comment_list_mimetype)
        self.assertEqual(rsp['stat'], 'ok')
        self.assertIn('commit_comments', rsp)
        self.assertEqual(len(rsp['commit_comments']), 1)
        self.assertEqual(rsp['commit_comments'][0]['text'], comment_text)
        self.assertTrue(rsp['commit_comments'][0]['issue_opened'])
...
class ResourceItemTests(CommentItemMixin, ReviewRequestChildItemMixin,
    ...
    fixtures = ['test_users']
    sample_api_url = \
        'review-requests/<id>/reviews/<id>/commit-message-comments/<id>/' # TODO: change this
    resource = resources.review_commit_comment
    ...
    def setup_review_request_child_test(self, review_request):
        review = self.create_review(review_request, user=self.user)
        comment = self.create_commit_message_comment(review)

        return (get_review_commit_comment_item_url(review, comment.pk),
                commit_message_comment_item_mimetype)
    ...
    def check_delete_result(self, user, comment, review):
        self.assertNotIn(comment, review.commit_comments.all())

    def test_delete_with_does_not_exist_error(self):
        """Testing the
        DELETE review-requests/<id>/reviews/<id>/commit-message-comments/<id>/ API
        with Does Not Exist error
        """
        review_request = self.create_review_request(publish=True)
        review = self.create_review(review_request, user=self.user)

        self.api_delete(get_review_commit_comment_item_url(review, 123),
                        expected_status=404)
    ...
    def check_put_result(self, user, item_rsp, comment, *args):
        comment = CommitMessageComment.objects.get(pk=comment.pk)
        self.assertEqual(item_rsp['text_type'], 'plain')
    ...
    def test_put_with_issue(self):
        """Testing the
        PUT review-requests/<id>/reviews/<id>/commit-message-comments/<id>/ API
        ...
        rsp = self.api_put(
            get_review_commit_comment_item_url(review, comment.pk),
            {'issue_opened': False},
            expected_mimetype=commit_message_comment_item_mimetype)
        self.assertEqual(rsp['stat'], 'ok')
        self.assertFalse(rsp['commit_message_comment']['issue_opened'])
    ...
    def test_put_issue_status_before_publish(self):
        ...
        self.assertEqual(rsp['commit_message_comment']['issue_status'], 'open')
    ...
    def test_put_issue_status_after_publish(self):
        ...
        rsp = self.api_put(
            ...
            expected_mimetype=commit_message_comment_item_mimetype)
        self.assertEqual(rsp['stat'], 'ok')
        self.assertEqual(rsp['commit_message_comment']['issue_status'], 'resolved')
    ...
    def test_put_issue_status_by_issue_creator(self):
        ...
        rsp = self.api_put(
            ...
            expected_mimetype=commit_message_comment_item_mimetype)
        self.assertEqual(rsp['stat'], 'ok')
        self.assertEqual(rsp['commit_message_comment']['issue_status'], 'dropped')
