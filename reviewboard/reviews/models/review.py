...
class Review(models.Model):
    ...
    untouched_comments = models.ManyToManyField(
        UntouchedComment,
        verbose_name=_("untouched comments"),
        related_name="review",
        blank=True
    )


    commit_message_comments = models.ManyToManyField(
        CommitMessageComment,
        verbose_name=_('commit message comments'),
        related_name='review',
        blank=True)
    ...
    def publish(self, user=None, trivial=False, to_owner_only=False,
        ...
        commits_to_incr_comment_counts = []

        for i in self.commit_message_comments.all():
            commits_to_incr_comment_counts.append(i.commit_id)

        for diffset in self.review_request.get_diffsets():
            diffset.incremenent_commit_comment_counts(commits_to_incr_comment_counts)
        # self.review_request.get_diffsets()[0].incremenent_commit_comment_counts(commits_to_incr_comment_counts)
        ...
        self.untouched_comments.update(timestamp=self.timestamp)
        self.commit_message_comments.update(timestamp=self.timestamp)
    ...
    def delete(self):
        ...
        self.untouched_comments.all().delete()
        self.commit_message_comments.all().delete()
    ...
    def get_all_comments(self, **kwargs):
        ...
        return (list(self.comments.filter(**kwargs)) +
                ...
                list(self.untouched_comments.filter(**kwargs)) +
                list(self.commit_message_comments.filter(**kwargs)))
    ...
    def has_comments(self, only_issues=False):
        ...
        qs = [
            ...
            self.untouched_comments,
            self.commit_message_comments,
        ]
