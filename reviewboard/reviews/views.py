...
from django.views.generic.base import (ContextMixin, RedirectView,
                                       TemplateView, View)
from djblets.conditions import values
...
from reviewboard.print.views import PrintView

from reviewboard.reviews.codeviewer_utils import get_file_tree, extract_file_path, get_original_code_file, \
    get_bread_crumbs, get_blob_renderer, enrich_untouched_in_comments_context_by_review_request, add_blob_comments, \
    build_untouched_comment_fragments
...
# Team17:
# New view for read status history
class ReadstatusHistoryView(ReviewRequestViewMixin,
                              UserProfileRequiredViewMixin,
                              ETagViewMixin,
                              TemplateView):
    template_name = 'reviews/readstatus_history.html'
    ...
    def __init__(self, **kwargs):
        ...
        """
        super(ReadstatusHistoryView, self).__init__(**kwargs)
    ...
    def get_context_data(self, **kwargs):
        ...
        # backend
        read_status_history = []
        read_status = ReadStatus.objects.filter(review_request=review_request.id, user=request.user.id)
        filesets = {}
        for ele in read_status:
            if ele.file_name not in filesets:
                filesets[ele.file_name] = [ele]
            else:
                filesets[ele.file_name].append(ele)
            ...
            for i in range(len(fileset)):
                if i == 0:
                   prev = 0
                    ...
                    prev = fileset[i-1].status
                file_dict = {}
                file_dict["revision"] = fileset[i].revision_id
                file_dict["name"] = fileset[i].file_name
                file_dict["prev"] = prev
                file_dict['curr'] = fileset[i].status
                file_dict['time'] = fileset[i].timestamp
                read_status_history.append(file_dict)

        sorted_history = sorted(read_status_history,key = lambda x: x['time'])[::-1]

        context = \
            super(ReadstatusHistoryView, self).get_context_data(**kwargs)
        context.update(make_review_request_context(request, review_request, is_history=True))
        context.update({
            'blocks': self.blocks,
            'draft': data.draft,
            'review_request_details': data.review_request_details,
...
class ReviewsDiffViewerView(ReviewRequestViewMixin,
    ...
    def get_context_data(self, **kwargs):
        ...
        # Team 17
        # Get current reviewer's all revision history
        review_request_id = kwargs.get("review_request_id")
        review_request = ReviewRequest.objects.filter(id=review_request_id)[0]
        ...
        target_people_lst = list(target_people)
        user_name_lst = []
        user_name_id =[]
        for user in target_people_lst:
            user_name_lst.append(user.username)
            user_name_id.append(user.id)

        # Team 17
        # Get current reviewer's all revision history
        read_status_history = {}
        for i in range(len(user_name_id)):
            read_status = ReadStatus.objects.filter(review_request=review_request_id, user=user_name_id[i])
            fileset = {}
            for ele in read_status:
                if ele.file_name in fileset:
                    if ele.timestamp >= fileset[ele.file_name].timestamp:
                        fileset[ele.file_name] = ele
                else:
                    fileset[ele.file_name] = ele
            file_status = {}
            for obj in fileset.values():
                file_status[obj.file_name] = obj.status
            read_status_history[user_name_lst[i]] = file_status

        read_status_history = json.dumps(read_status_history)
        ...
        for f in context['files']:
            ...
            data = {
                # Team 17
                # Pass read status history to frontend
                'read_status': read_status_history,
                'newfile': f['newfile'],
                'binary': f['binary'],
                'deleted': f['deleted'],
                'id': filediff.pk,
        ...
        #obtain review and review id
        diff_context['review_id'] = context['review'].id if context['review'] else -1

        self.enrich_untouched_comments_in_context(context)
        return context

    def enrich_untouched_comments_in_context(self, context):
        """
            get untouched files list and untouched comments according to the review requests
        """

        modified_files = []
        if context['diffset']:
            for filediff in context['diffset'].cumulative_files:
                modified_files.append(filediff.source_file)

        enrich_untouched_in_comments_context_by_review_request(self.review_request, len(context['files']),
                                                               set(modified_files), self.request.path, context)


class ReviewsCodeBrowserFragmentView(ReviewRequestViewMixin, View, ContextMixin):
    template_name = 'reviews/code_viewer_fragment.html'

    def get(self, request, *args, **kwargs):

        extract_file_path(self.request, kwargs)
        response = ""
        try:
            renderer_settings = self._get_renderer_settings(**kwargs)
            context = super(ReviewsCodeBrowserFragmentView, self).get_context_data(**kwargs)
            context.update({
                'review_request': self.review_request,
            })

            blob_data = self.get_blob_data(kwargs['code_path'])
            context.update(blob_data)
            ...
            logging.exception('%s.get: Error when processing blob : %s',
    ...
    def get_blob_data(self, code_path):
        diffsets = self.review_request.get_diffsets()
        num_diffs = len(diffsets)
        ...
        # what if latest_diffset == None?
        if latest_diffset:
            for filediff in latest_diffset.cumulative_files:
                modified_files.append(filediff.source_file)

        original_blob_data = get_original_code_file(self.review_request,
                                           code_path,
                                           set(modified_files), self.request.GET['hashcode'])

        return original_blob_data


    def create_renderer(self, context, renderer_settings, blob_data,
                        *args, **kwargs):
        return get_blob_renderer(blob_file=blob_data,
...
class ReviewsCodeBrowserView(ReviewRequestViewMixin,
                             UserProfileRequiredViewMixin,
                             TemplateView):
    """A view for the repository code browsing page.

    This page shows information of the code repository based on the review request.
    """

    template_name = 'reviews/code_browser.html'
    ...
    def __init__(self, **kwargs):
        ...
        self.diffset = None
        self.interdiffset = None
        super(ReviewsCodeBrowserView, self).__init__(**kwargs)

    def get(self, request, revision=None, *args,
            **kwargs):
        """Handle HTTP GET requests for the code browser view.

        This will look up the review request and parse the URL pattern
        to extract requested code repo path.
        ...
        """
        # review_request = self.review_request
        extract_file_path(self.request, kwargs, revision)

        return super(ReviewsCodeBrowserView, self).get(
            request=request,
            *args,
            **kwargs)
    ...
    def get_context_data(self, **kwargs):
        ...
        # Build the final context for the page.
        context = super(ReviewsCodeBrowserView, self).get_context_data(**kwargs)
        context.update({
            'close_description': close_info['close_description'],
            'close_description_rich_text': close_info['is_rich_text'],
            'close_timestamp': close_info['timestamp'],
        ...
        self.enrich_context(context, kwargs, latest_diffset)
        url = self.review_request.get_absolute_url()
        context.update(make_review_request_context(self.request,
                                                   self.review_request))

        return context

    def enrich_context(self, context, kwargs_dict, latest_diffset):
        modified_files = []
        ...
        if not kwargs_dict['is_blob']:
            repo_data = get_file_tree(self.review_request,
                                      kwargs_dict['code_path'],
                                      kwargs_dict['revision'],
                                      set(modified_files), latest_diffset.base_commit_id,
                                      self.request.path)
        else:
            path_lst = [self.review_request.repository.name]
            path_lst += kwargs_dict['code_path'].split("/")
            is_touched = False
            if kwargs_dict['code_path'] in set(modified_files):
                is_touched = True
            repo_data = {
                "is_blob": kwargs_dict['is_blob'],
                'path_list': path_lst,
                'blob_content_url': kwargs_dict['blob_content_url'],
                'is_touched': is_touched,
                'diff_url': kwargs_dict['diff_url'] if is_touched else ""
            }
            add_blob_comments(kwargs_dict['code_path'], self.request.GET.get('hashcode'), context, self.review_request)
        bread_crumbs = get_bread_crumbs(repo_data['path_list'], self.request.path)
        context_code = {
            "is_blob" : kwargs_dict['is_blob'],
            "code_path" : kwargs_dict['code_path'],
            'blob_content_url' : kwargs_dict['blob_content_url'] if 'blob_content_url' in kwargs_dict else '' ,
            'untouched_comments': context['untouched_comments'] if 'untouched_comments' in context else {},
            'review_id': context['review'].id if context['review'] else -1,
            "hashcode" : self.request.GET.get('hashcode') if kwargs_dict['is_blob'] else "",
        }
        ...
        if repo_data:
            context.update(repo_data)
            context.update(bread_crumbs)
...
class CommentDiffFragmentsView(ReviewRequestViewMixin, ETagViewMixin,
    ...
    def get_etag_data(self, request, comment_ids, *args, **kwargs):
        ...
        # self.untouched_comments = get_list_or_404(UntouchedComment, q)

        latest_timestamp = get_latest_timestamp(
            [comment.timestamp for comment in self.comments]
        )
...
class UntouchedCommentFragmentsView(ReviewRequestViewMixin, ETagViewMixin,
                               ContextMixin, View):
    ...
    def get_etag_data(self, request, comment_ids, *args, **kwargs):
        ...
        latest_timestamp = get_latest_timestamp(
            [untouched_comment.timestamp for untouched_comment in self.untouched_comments]
        )
    ...
    def get(self, request, **kwargs):
        ...
        context = \
            super(UntouchedCommentFragmentsView, self).get_context_data(**kwargs)
        context.update({
            'request': request,
            'user': request.user,
        })
        ...
        payload = io.BytesIO()
        comment_entries = build_untouched_comment_fragments(
            untouched_comments=self.untouched_comments,
            context=context,
            review_request=self.review_request,
            comment_template_name=self.comment_template_name,
            error_template_name=self.error_template_name)[1]
...
class ReviewsDiffFragmentView(ReviewRequestViewMixin, DiffFragmentView):
    ...
    def create_renderer(self, diff_file, context, *args, **kwargs):
        ...
        renderer = super(ReviewsDiffFragmentView, self).create_renderer(
            diff_file=diff_file, context=context, *args, **kwargs)
        ...
        # Team 17: Extract all read status for the current user in this review
        # request in this revision for this file. This information is then 
        # forwarded to the renderer's context
        read_status = ReadStatus.objects.filter(user_id=self.request.user.id,review_request_id=context['review_request_id'],
                                                revision_id=context['revision'],file_name=context['file_name'])
        status = 0
        if read_status.count() > 0:
            for ob in read_status:
                status = ob.status
        renderer.extra_context.update(
            {'f_read_status': status}
        )
        return renderer
    ...
    def get_context_data(self, **kwargs):
        return {
            ...
            'revision': kwargs.get("diffset").revision,
            'file_name': kwargs.get("filediff").source_file,
        }
...
# TEAM 12- PaperTrails
class ReviewRequestPrintView(ReviewRequestDetailView,
    ...
    def __init__(self, **kwargs):
        ...
        Args:
            **kwargs (dict):
                Keyword arguments passed to :py:meth:`as_view`.
        """
        super(ReviewRequestPrintView, self).__init__(**kwargs)
        self.draft = None
        self.diffset = None
        self.interdiffset = None
        self.collapse_diffs = False
        self.issues_exist = True
        self.issues_flag_default = True

    def get_issue_flags(self, key, request):
        '''Fetches get request for a given key in the request
            and converts it to a boolean if it there

        Args:
            key (string):
                The key for an html id
    ...
    def checked_files(self, request):
        '''Fetches get request to generate a list of files
            to render in the file diffs.
        ...
        Returns:
            A list of files 
        '''
        return [key for key, value in request.GET.items() if value == 'yes' or value == 'on']

    
    def get_diff_dict(self, data, context):
        '''Creates dictionary of file diffs
        ...
        Returns:
            dictionary:
                Key is the file name of the file that was changed
                Value is the diff file as a string 
        '''
        slider_end = context["slider_end"]
        comments = context['comments_code']
        split_data = data.split(b'diff --git')
        parsed_diffs = {}

        for data in split_data:
            str_data = data.decode("utf-8") 

            # Get filename of diff
            head = str_data.partition("\n")[0]
            file_name = head.split(" ")[-1][2:]
            ...
            if file_name != '':
                parsed_diffs[file_name] = { "filename": file_name, "raw_diff": str_data}
        
        for comment in comments:
            filename = comment.filediff.dest_file_display
            file = parsed_diffs.get(filename)
            flag = context["with_file_flag"]
            same_revision = comment.filediff.diffset.revision == int(slider_end)

            if (file is not None) and flag and same_revision:
                parsed_diffs[filename]["raw_diff"] += "\n Comment from {0}: \n ({1}) {2} \n".format(comment.review_obj._user_cache, comment.timestamp, comment.text)
    ...
    def get(self, request, *args, **kwargs):
        ...
        Returns:
            ...
            custom update payload content.
        '''
        # Get values of revision slider via text box
        if (len([(key, value) for key, value in request.GET.items()])):
            self.issues_flag_default = False

        revision_range = request.GET.get('amount')
        
        # If this is not the first loading of the page, pass in end_range parameter to get_diff function
        if revision_range:
            revision_range = revision_range.split()
            start_range, end_range = revision_range[0], revision_range[2]

            self.diffset = self.get_diff(end_range)

            if start_range != '0':
                self.interdiffset = self.get_diff(start_range)

        else:
            self.diffset = self.get_diff()

        kwargs['diffset'] = self.diffset
        # Changing self.interdiffset changes the files list 
        kwargs['interdiffset'] = self.interdiffset
        context = self.get_context_data(**kwargs)

        # Check if any issues exist
        if all(value == 0 for value in context['issue_counts'].values()):
        ...
        # Get length of diffset for the slider max
        len_diffset = len(context['diffsets'])
        context.update({"len_diffset": len_diffset}) 

        # Set slider values
        if revision_range:
            context.update({'slider_start': start_range, 'slider_end': end_range})
        else:
            context.update({'slider_start': 0, 'slider_end': len_diffset})
       
        # With file comments flag
        with_file_flag = True if request.GET.get('with_file') == 'with_file' else False

        context.update({
            'with_file_flag': with_file_flag
        })
        
        context['diffset_pair'] = (self.diffset, self.interdiffset)
        tool = self.review_request.repository.get_scmtool()
        data = tool.get_parser(b'').raw_diff(self.diffset)
        parsed_diffs = self.get_diff_dict(data, context)
        
        context.update({"data": parsed_diffs})

        # Issues flags
        open_flag = self.get_issue_flags("open", request)
        res_flag = self.get_issue_flags("resolved", request)
        drop_flag = self.get_issue_flags("dropped", request)
        verif_flag = self.get_issue_flags("verif", request)
        
        issue_flag_dict = {'O': open_flag,'R':res_flag,'D':drop_flag,'A': verif_flag,'B':verif_flag}
        
        file_dests = [file['dest_filename'] for file in context['files']]
        
        # Process checked files
        checked_files = self.checked_files(request)  
        num_files = len(checked_files)          

        filepath_list = [parsed_diffs[file_name]["filename"] for file_name in parsed_diffs]
        node = generate_tree_structure(filepath_list)
        file_form_html = generate_tree_html(node, self.issues_flag_default, set(checked_files))

        if self.issues_flag_default and num_files == 0:
        ...
        context.update({
            ...
            'issues_exist': self.issues_exist,
            'issue_flag_dict': issue_flag_dict,
            'issues_flag_default': self.issues_flag_default,
            'file_form_html': file_form_html
        })

        filename = os.path.join(settings.REVIEWBOARD_ROOT, 'templates/reviews/print.html')
        response  = render(request, filename, context)
        return response 

    def get_context_data(self, **kwargs):
        '''Populates the context dictionary with comments
            and issues
        ...
        '''
        context = ReviewRequestDetailView.get_context_data(self, **kwargs)
        issues = {}
        issue_types = {'O':'Open','R':'Resolved','D':'Dropped','A':'Verifying Resolved','B':'Verifying Dropped'}
        for issue in context['issues']:
            for issue_type in issue_types:
                if issue_type not in issues:
                    issues[issue_type] = []
                if issue.issue_status == issue_type:
                    issues[issue_type].append(issue)
        
        comments_initial = []
        comments_change_entry = []
        comments_code = []
        comments_general = []
        comments_file_attachment = []
        comments_screenshot = []
        for entry_type in context['entries']:
            for comment in context['entries'][entry_type]:
                if type(comment) == InitialStatusUpdatesEntry: 
                    comments_initial.append(comment)
                elif type(comment) == ChangeEntry: 

                    comments_change_entry.append(comment)
                elif type(comment) == ReviewEntry: 
                    # print(vars(comment))
                    if comment.comments['diff_comments']:
                        comments_code.append(comment.comments['diff_comments'][0])
                    elif comment.comments['general_comments']:
                        comments_general.append(comment.comments['general_comments'][0])
                    elif comment.comments['file_attachment_comments']:
                        comments_file_attachment.append(comment.comments['file_attachment_comments'][0])
                    elif comment.comments['screenshot_comments']:
                        comments_screenshot.append(comment.comments['screenshot_comments'][0])

        context.update({'issues':issues,
                        'issue_types':issue_types,
                        'comments_initial':comments_initial,
                        'comments_change_entry':comments_change_entry,
                        'comments_code':comments_code,
                        'comments_general':comments_general,
                        'comments_file_attachment':comments_file_attachment,
                        'comments_screenshot':comments_screenshot,
                        })
