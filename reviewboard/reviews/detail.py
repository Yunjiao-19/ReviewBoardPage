        ...
        if self.reviews:
            ...
            for model, review_field_name, key, ordering in (
                ...
                (CommitMessageComment,
                 ...
                 'commit_message_comments',
                 ('commitmessagecomment__timestamp',))):
            ...
            for comment_list in self.review_comments.values():          
                for comment in comment_list:
                    if 'parentComments' in comment.extra_data:
                        check = comment.extra_data['parentComments']
                    else:
                        check = f'{{}}'

                    if (check != f'{{}}' and comment.get_review().public):
                        parent_dict = json.loads(check)
                        for parent in parent_dict.values():
                            parent_id = parent['id'] 
                            if (parent_id in self.child_comments.keys()):
                                self.child_comments[parent_id].append(comment) 
                            else:
                                self.child_comments[parent_id] = [comment]
...
class DiffCommentsSerializerMixin(object):
    ...
    def serialize_untouched_comments_js_model_data(self, untouched_comments):
        """Serialize information on diff comments for JavaScript models.
        ...
        Returns:
            ...
            The serialized data for the JavaScript model.
        """
        untouched_comments_data = []
        for comment in untouched_comments:

            untouched_comments_data.append((six.text_type(comment.pk), 1))


        return untouched_comments_data
...
class ReviewEntry(ReviewEntryMixin, DiffCommentsSerializerMixin,
        ...
        for review in data.reviews:
            ...
            for comment in data.review_comments.get(review.pk, []):
                ...
                if (check == f'{{}}'): 
                    ...
                    if (comment.pk in data.child_comments.keys() and 
                        comment.comment_type != 'general'):
                            comment.children = data.child_comments[comment.pk]

            all_lists_empty = True
            for list in entry.comments.values():
                if list:
    ...
    def get_js_model_data(self):
        ...
        untouched_comments_data = self.serialize_untouched_comments_js_model_data(
            self.comments['untouched_comments']
        )

        if diff_comments_data:
            model_data['diffCommentsData'] = diff_comments_data
        
        if untouched_comments_data:
            model_data['untouchedCommentsData'] = untouched_comments_data
