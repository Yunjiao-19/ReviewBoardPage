...
def make_review_request_context(request, review_request, extra_context={},
                                is_diff_view=False, is_history=False):
    ...
    tabs = [
        ...
        {
            'text':_('Code'),
            'url': review_request.get_absolute_url()+'code/',
        },
    ]
    ...
    # Team17:
    # Add history tag url to context
    tabs.append({
        'active': is_history,
        'text': _('Review History'),
        'url': review_request.get_absolute_url() + 'history'
    })
