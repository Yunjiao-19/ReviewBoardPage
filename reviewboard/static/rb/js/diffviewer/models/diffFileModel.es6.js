...
RB.DiffFile = Backbone.Model.extend({
    defaults: {
        ...
        readStatus: null, //Team17: new field for read status
    },


    url() {
        let url = SITE_ROOT + (this.get('localSitePrefix') || '') + this.get('url');
        if (!this.isNew()) {
            url += this.get('name') + '/';
        }
        return url;
    ...
    parse(rsp) {
        return {
            ...
            readStatus: rsp.read_status, //Team17: new field for readStatus
        };
    },
});
