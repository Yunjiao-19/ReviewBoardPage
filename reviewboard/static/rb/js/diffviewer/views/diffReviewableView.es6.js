...
RB.DiffReviewableView = RB.AbstractReviewableView.extend({
    ...
    events: {
        ...
        'click .readstatus': '_onReadStatusRadioBtnClicked', 
        'click .update-btn': '_onUpdateBtnClicked',
    ...
    initialize() {
        ...
        this.files = new RB.DiffFile(); // Team17
        ...
        this._readstatus = "unread";
        this._readstatusEnabled = true;
   ...
   /**
     * Team17:
     *
     * Change read status radio display when the radio is clicked.
     */ 
    _onReadStatusRadioBtnClicked(e) {
        e.stopPropagation();
        if (this._readstatusEnabled) {
            if ($(e.target).is(':radio')) {
                this._readstatus = $(e.target).val();
            }
        }
    },
     ...
     * Team17:
     *
     * Make a post require to backend when the radio is selected and update button is pressed.
     */ 
    _onUpdateBtnClicked(e) {
        e.stopPropagation();
        if (this._readstatusEnabled) {
            var to_number = {"read": 2, "tentative": 1, "unread": 0};

            this.files.set({
                status: to_number[this._readstatus],
                file_name: e.target.parentNode.getAttribute('name'),
                revision_id: this.model.get('revision'),
                url: 'api/review-requests/'+this.model.get('reviewRequest').id+'/read-status/'
            });
            
            this.files.save(null, {
     ...
     * Team17:
     *
     * Disable the radio and update button.
     */ 
    disableReadstatus() {
        this._readstatusEnabled = false
        $( "input[id=status_unread]" ).attr("disabled",true);
        $( "input[id=status_tentative]" ).attr("disabled",true);
        $( "input[id=status_read]" ).attr("disabled",true);
        $( "button[id=update_button]" ).attr("disabled",true);
    },

    deleteReadstatus() {
        $( "div[id=read_status_div]" ).hide();
    },
