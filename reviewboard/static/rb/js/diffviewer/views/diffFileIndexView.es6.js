...
RB.DiffFileIndexView = Backbone.View.extend({
    ...
    _itemTemplate: _.template(dedent`
        <tr class="loading
         ...
         <!-- Team17: New hmtl tag for color block and text for read status -->
         
         <td class="vl">
            <div href="fffff">
                <% if (readstatus == 2) { %>read<% } %>
                <% if (readstatus == 1) { %>tentative<% } %>
                <% if (readstatus == 0 || readstatus == null) { %>unread<% } %>
            </div>
         </td>

         <td>
          <a class = "readstatus_square  
          <% if (readstatus == 2) { %> green<% } %>
          <% if (readstatus == 1) { %> yellow<% } %>
          <% if (readstatus == 0 || readstatus == null) { %> red<% } %>"></a> 
         </td>
    ...
    update() {
        ...
        this.collection.each(file => {
            ...
            // console.log(file.get('readStatus'));
