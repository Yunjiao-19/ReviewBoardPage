 ...
 * a new comment.
 */
RB.CodeTextRowSelector = Backbone.View.extend({
    ...
    initialize(options) {
        ...
        this.untouchedFile = options.untouchedFile;
    ...
    */
    ...
    getRowsForRange(beginLineNum, endLineNum, minRowIndex) {
        const beginRowEl = $(this.el).find(`tr[line^=${beginLineNum}]`)
        const endRowEl = beginRowEl
    ...
    _end($row) {
        ...
        RB.CodeCommentDialogView.create({
            'comment':this.model,
            'untouchedFile': this.untouchedFile,
            'line': this._endLineNum
        })
