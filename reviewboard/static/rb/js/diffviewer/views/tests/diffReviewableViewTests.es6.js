suite('rb/diffviewer/views/DiffReviewableView', function() {
    ...
    describe('Incremental expansion', function() {
        ...
        describe('Expanding', function() {
            ...
            // Team17:
            // Set the radio and button to be enabled at first.
            describe('Read Status', function() {
                it('Initial read status', function() {
                    expect($( "input[id=status_unread]" ).is(':disabled')).toBe(false)
                    expect($( "input[id=status_tentative]" ).is(':disabled')).toBe(false)
                    expect($( "input[id=status_read]" ).is(':disabled')).toBe(false)
                    expect(view._readstatus).toBe("unread");
                    expect($( "input[id=status_unread]" ).is(':checked')).toBe(false)
                    expect($( "input[id=status_tentative]" ).is(':checked')).toBe(false)
                    expect($( "input[id=status_read]" ).is(':checked')).toBe(false)
                });
