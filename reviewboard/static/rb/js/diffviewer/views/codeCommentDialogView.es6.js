(function() {
    ...
    const CommentsListView = Backbone.View.extend({
            ...
            <li class="<%= itemClass %>">
             <h2>
              ...
              <span class="actions">
               ...
               <a class="comment-list-reply-action" href="<%= comment.url %>"
                  ><%- replyText %></a>
              </span>
             </h2>
        ...
        setComments(comments, replyType) {
            ...
            comments.forEach(comment => {
                const commentID = comment.firstline;
     ...
     */
    RB.CodeCommentDialogView = Backbone.View.extend({
        ...
        save() {
             ...
             */
            const review_url = this.model.attributes.reviewRequest.url()+'reviews/'
            const review_request_id = this.model.attributes.reviewRequest.id
            const id = this.options.code_options.comment.reviewId
            const line = this.options.code_options.line
            const text = this.model.attributes.text
            const untouched_file = this.options.code_options.untouchedFile
            const file_path = untouched_file.path
            const source_file_hashcode = untouched_file.hashcode
            ...
            // filediff_id needs to be dynamic
            const save_params = {
                api_format: 'json',
                filediff_id: 1,
                num_lines: 1,
                first_line: line,
                include_text_types: 'html,raw,markdown',
                issue_opened: false,
                text_type: 'plain',
                text: text,
                'extra_data.require_verification': false,
                expand: 'filediff,interfilediff',
                file_path: file_path,
                source_file_hashcode: source_file_hashcode,
            ...
            if(id===-1){
                this.saveUntouchedComment(review_request_id,review_url,review_params,save_params);
            }else{
                const review_id = id
                const url = `/api/review-requests/${review_request_id}/reviews/${review_id}/untouched-comments/`
        ...
        saveUntouchedComment(review_request_id,review_url,review_params,save_params){
            return new Promise((resolve,reject)=>{
                ...
                return Promise.resolve(res.review.id)
            }).then((review_id)=>{
        ...
        create: function(options={}) {
            ...
            const dlg = new RB.CodeCommentDialogView({
