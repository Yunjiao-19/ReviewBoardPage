...
const itemTemplate = _.template(dedent`
    <tr<% if (rowClass) { %> class="<%- rowClass %>"<% } %>>
     ...
     <% } %>
     <td>
        <span title="Check Reviews tab for comments"  data-commitid="<%- commit.id %>" class="commit-comment-indicator"><%- commit.get('numComments') %></span>
        <a href="#"
           class="commit-comment-add"
           id="commit-comment-action"
           data-commitid="<%- commit.id %>"
           aria-role="button">
         <span class="fa fa-comments-o" title="Add Comment"></span>
         Add Comment
        </a>
     </td>
            ...
            .append(headerTemplate(_.extend(
                {
                    ...
                    commitCommentText: gettext('Commit Comment'),
                },
                commonContext
            )));
