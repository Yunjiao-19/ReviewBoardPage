 ...
 * 
 *     parentComments (Array of RB.BaseComment):
 *         Parent comments associated with the comment. 
 *
 ...
 * 
 *     otherComments (Array of RB.BaseComment):
 *         List of all of a review's comments with a non-empty text field.
 *
...
RB.CommentEditor = Backbone.Model.extend(_.defaults({
    ...
    initialize() {
        ...
        this.on('change:parentComments', this._updateState, this);
    ...
    save(options={}, context=undefined) {
        ...
        let parentCommentObj = Object.assign({}, this.get('parentComments'));
        extraData.parentComments = JSON.stringify(parentCommentObj);

        const comment = this.get('comment');
        comment.set({
            text: this.get('text'),
            issueOpened: extraData.parentComments === "{}"
                         ? this.get('openIssue')
                         : false,
            extraData: extraData,
    ...
    _updateFromComment() {
        ...
        if (comment) {
            ...
            this.set({
                ...
                parentComments: 
                    comment.get('extraData').parentComments === undefined 
                    ? []
                    : JSON.parse(comment.get('extraData').parentComments),
    ...
    _updateState() {
        ...
        const comment = this.get('comment');
        const parentComments = this.get('parentComments');
