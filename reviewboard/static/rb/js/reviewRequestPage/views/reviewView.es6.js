...
RB.ReviewRequestPage.ReviewView = Backbone.View.extend({
    ...
    render() {
        ...
        const untouchedCommentsData = this.entryModel.get('untouchedCommentsData');
        ...
        for (let j = 0; j < untouchedCommentsData.length; j++) {
            const untouchedCommentData = untouchedCommentsData[j];
            const pk = untouchedCommentData[0];
            const filediff_id = untouchedCommentData[1];
            const url = this.model.get('parentObject').get('reviewURL')+`_fragments/untouched-comments/${pk}/`
            const $container = document.querySelector(`#untouched_chunks_${pk}`)
            this._renderUntouchedFragment($container, url)
        }
    ...
    _renderUntouchedFragment(container, url){
        $.get(url,(data)=>{
