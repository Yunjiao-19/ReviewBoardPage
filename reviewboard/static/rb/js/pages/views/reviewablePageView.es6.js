...
RB.ReviewablePageView = RB.PageView.extend({
    events: _.defaults({
        ...
        'click #print-review-action': '_onPrintReviewClicked',
        'click #commit-comment-action': '_onAddCommitCommentClicked',
     ...
     *    false, always.
     */
    _onAddCommitCommentClicked(e) {
        const pendingReview = this.model.get('pendingReview');
        const comment = pendingReview.createCommitMessageComment(
            undefined,
            $(e.currentTarget)[0].dataset.commitid,
            RB.UserSession.instance.get('commentsOpenAnIssue'));

        this.listenTo(comment, 'saved',
                      () => {

                       const currentCommitID = $(e.currentTarget)[0].dataset.commitid;
                       const indicatorValue = $(".commit-comment-indicator[data-commitid="+currentCommitID+"]").text();
                       const incrementIndicator = parseInt(indicatorValue) + 1;
                       const indicatorUpdate = $(".commit-comment-indicator[data-commitid="+currentCommitID+"]").text(incrementIndicator);
    ...
    _onShipItClicked() {
       ...
       /**
     * Handle a click on the "Print Review" button.
     *
     * Navigates to a page with a printout of the review. 
     *
     * Returns:
     *    boolean:
     *    false, always.
     */
    _onPrintReviewClicked() {
        const reviewRequest = this.model.get('reviewRequest');
        window.location.href = reviewRequest.get('reviewURL') + 'print/';
