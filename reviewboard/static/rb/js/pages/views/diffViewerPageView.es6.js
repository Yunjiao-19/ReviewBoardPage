...
RB.DiffViewerPageView = RB.ReviewablePageView.extend({
    ...
    events: _.extend({
        ...
        'change select': '_onSelectUsername', // Team17
    ...
    _fileSeparator: _.template(dedent`
        <div style="margin-top: 80px; margin-bottom: 80px; font-size: 15px; text-align: center">
            <hr style="vertical-align:middle;" width="85%" color=#989797 SIZE=2 />
            Untouched Files
            <hr style="vertical-align:middle;" width="85%" color=#989797 SIZE=2 />
        </div>
    `),

    _fileRenderTemplate: _.template(dedent`
        <thead>
            <tr class="filename-row">
                <th colspan="4">
                    <a name="<% index %>" class="file-anchor"></a>
                    <%- filename %>
                    <span class="warning"><% if (is_touched) %> Obsolete Version </span>
                </th>
            </ tr>
        </thead>
    `),
    ...
    render() {
        ...
        this._$diffs.append(this._fileSeparator())

        //render untouched files
        if (this.model.untouchedFiles.length > 0){
            this.model.untouchedFiles.forEach(untouched_file=>{
                this._addUntouched(untouched_file)
                this.getRenderFragment(untouched_file)
            })
        }
        ...
        /*
         * Team17:
         * count the number of read and tentatively read file and display the number
         */
        get_readstatus_stats(){
            var stats = {read:0, tentative:0, unread:0, total:0};
            const status_name = ["unread", "tentative", "read"];
            this.model.files.each(file => {
                var username = $("select").val();
                var read_status = JSON.parse(file.get('readStatus'))[username][file.get('depotFilename')];
                stats[status_name[read_status]] += 1;
                stats["total"] += 1;
            });
            // console.log(stats);
            $("div[id=readstatus-stats]").text(`${stats.read} files read, ${stats.tentative} files tentative out of ${stats.total} files`);
            return stats;
        },
    ...
    // Team17:
    // Update the file index section when username is clicked.
    _onSelectUsername(e) {
        e.stopPropagation();        
        this._diffFileIndexView.update();
    ...
    _renderFileDiff(diffReviewable) {
        ...
        // Team17
        // Get some extra variable for read status
        const revision = this.model.revision.get('revision');
        const latestRevision = this.model.revision.get('latestRevision');
        const curInterdiffRevision = this.model.revision.get('interdiffRevision');
        if(revision != latestRevision && curInterdiffRevision != latestRevision) {
            diffReviewableView.disableReadstatus();
        }
        
        const targetPeople = this.model.get('reviewRequest').get('targetPeople');
        const username = RB.UserSession.instance.get('username');
        var invalidUser = true;
        for (const [key, value] of Object.entries(targetPeople)) {
        ...
        if (invalidUser) {
            diffReviewableView.deleteReadstatus();
        }
        // Team17 end
    ...
    */
    ...
    _addUntouched(untouched_file){
        this._$diffs.append(this._fileEntryTemplate({
            id: untouched_file.index,
            filename: untouched_file.path,
            newFile: false,
        }));
    },

    getRenderFragment(untouched_file, callbacks, context, options={}){
        const prefix = '#file_container_'
        const url = untouched_file.blob_content_url
        const template = this._fileRenderTemplate
        const selector = new RB.CodeTextRowSelector({
            el: $(prefix+untouched_file.index),
            model:this.model,
            untouchedFile: untouched_file
        })
        $.get(url,(data)=>{
            const $container =$(prefix+untouched_file.index)
            const untouchedFiles = this.model['untouchedFiles']?.filter((e)=>{
                return e.path === untouched_file.path
            })
            $container[0].innerHTML = template({
                filename: untouched_file.path,
                index: untouched_file.index,
                is_touched: untouchedFiles[0]?.is_touched || false
            })
            $container[0].innerHTML += data
            untouched_file?.comment?.forEach((content,index)=>{
                const commentBlockView = new RB.TextBasedCommentBlockView()
                content['count'] = index+1
                const publishedComments = untouched_file?.comment.filter((e)=>{
                    return e.firstline == content.firstline
                })
                commentBlockView.renderUntouchedComment(content,publishedComments)
                this.placeCommentBlock(selector,commentBlockView,content,untouched_file,publishedComments)
            });
    ...
    placeCommentBlock(selector,commentBlockView,comment,untouched_file,publishedComments){
        const firstRow = comment.firstline
        const numRows = comment.num_lines
        const rowEls = selector.getRowsForRange(
            firstRow,
            firstRow,
            undefined
        )
        ...
        commentBlockView.on('clicked', () => {
            RB.CodeCommentDialogView.create({
                comment:this.model,
                untouchedFile: untouched_file,
                line: firstRow,
                publishedComments: publishedComments || [],
                publishedCommentsType: untouched_file.comment[0].comment_type,
            })
        });
    },
