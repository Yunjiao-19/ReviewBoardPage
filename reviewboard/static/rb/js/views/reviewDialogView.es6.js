...
const BaseCommentView = Backbone.View.extend({
    ...
    events: {
        ...
        'click .anchor': '_onEditLinkClicked',
    },
    ...
    editorTemplate: _.template(dedent`
        <div class="edit-fields">
         ...
         <div id="list2" class="dropdown-check-list edit-field" tabindex="100">
    ...
    render() {
        ...
        this.$issueOpened = this.$('.issue-opened')
            .prop('checked', this.model.get('issueOpened') && 
                (this.model.get('extraData').parentComments === "{}" || 
                this.model.get('extraData').parentComments === undefined || 
                !('parentComments' in this.model.get('extraData'))))
            .change(() => {
                if (this.model.get('extraData').parentComments === "{}" || 
                    this.model.get('extraData').parentComments === undefined || 
                    !('parentComments' in this.model.get('extraData'))) {
        ...
        this.listenTo(this.inlineEditorView, 'complete', value => {
            ...
            if (this.model.isNew()) {
                 ...
                 */
                attrs.push('extra_data.require_verification', 
                'extra_data.parentComments', 'issueOpened');
            }
        ...
        this.linkCommentsList = new EditLinkCommentsView({
            el: this.$('.items'),
        });
        ...
        let parentIDs = [];
        let parentJSON = this.model.get('extraData').parentComments;
        if (parentJSON) {
            const parentComments = JSON.parse(parentJSON);
            for (let comment in parentComments) {
                parentIDs.push(parentComments[comment].id.toString());
            }
        }
        ...
        if (!(this.model.get('extraData').parentComments === "{}" || 
            this.model.get('extraData').parentComments === undefined || 
            !('parentComments' in this.model.get('extraData')))) {
            this.model.set('issueOpened', false);
        }
     ...
     */
      _get_linked_comments() {
        const $editFields = this.$('.edit-fields');
        this._$checkList = $editFields.find('#list2');
        this._$itemList = $editFields.find('.items');
        let input_list = this._$itemList.find('input');
        let all_comments = this._otherComments;
        let checked_id_list = [];
        let linked_comments = [];
        ...
        let exData = _.clone(this.model.get('extraData'));
        let parentCommentObj = Object.assign({}, linked_comments);
        exData.parentComments = JSON.stringify(parentCommentObj);;
        this.model.set('extraData', exData);
        this.model.save({
            attrs: ['extra_data.parentComments'],
        });
    },
                ...
                rList.reviews.forEach(review => {
                    RB.apiCall({
                        ...
                        url: '/api/review-requests/' + 
                             String(reviewRequest.id) + '/reviews/' + 
                            ...
                            rsp.file_attachment_comments.forEach(comment => {
                                ...
                                if (cond1 && 
                                 ...
                                 this.linkCommentsList.addComment(comment, 
                                    parentIDs, this);  
                                 let totalComments = this._otherComments;
 ...
 */
 const EditLinkCommentsView = Backbone.View.extend({
    itemTemplate: _.template(dedent`
        <li>
            <label for="<% commentId %>"><input id="<%- commentId %>" 
            type="checkbox" /><%- comment.text %></label>
        </li>
    `),
    ...
    /**
     * Add a checkbox option to the list of comments in the Link to Other Comment(s) menu.
     *
     * Args:
     *     inputComment (RB.FileAttachmentComment, RB.ScreenshotComment, or RB.DiffComment):
     *         The comment whose text will be used to add a checkbox to the menu.
     */
    addComment(inputComment, parentIDs, editor) {
        let inputCommentID = inputComment.id;
...
/**
 * Displays a view for commit comments.
 */
 const CommitMessageCommentView = BaseCommentView.extend({
    thumbnailTemplate: null,
});
...
RB.ReviewDialogView = Backbone.View.extend({
    ...
    template: _.template(dedent`
        ...
        <ol id="review-dialog-commit-comments" class="review-comments"></ol>
    ...
    initialize(options) {
        ...
        this._$lastCommitMessageComment = null;

        this._commitMessageCommentsCollection = new RB.ResourceCollection([], {
            model: RB.CommitMessageComment,
            parentResource: this.model,
        });

        this.listenTo(this._commitMessageCommentsCollection, 'add', comment => {
            const view = new CommitMessageCommentView({ model: comment });
            this._renderComment(view, this._$commitMessageComments);
        });
     ...
     */
    close() {
        this._commentViews.forEach(view => {
            view._get_linked_comments();
        });
    ...
    render() {
        ...
        this._$commitMessageComments = this.$('#review-dialog-commit-comments');
        this._$screenshotComments =
    ...
    _loadComments() {
        const collections = [
            ...
            this._commitMessageCommentsCollection
        ];
