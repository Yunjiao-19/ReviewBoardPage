...
RB.TextBasedCommentBlockView = RB.AbstractCommentBlockView.extend({
    ...
    renderUntouchedContent(comment){
        this.$el.html(this.template({
            anchorName: `anchor-${comment.count}-${comment.firstline}`,
        }));

        const countEle = $(this.$el).find('.commentflag-count')[0]
        countEle.innerHTML = comment.count
