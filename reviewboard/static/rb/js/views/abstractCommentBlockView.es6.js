RB.AbstractCommentBlockView = Backbone.View.extend({
    ...
    renderUntouchedComment(comment,publishedComments){
        this._$tooltip = $.tooltip(this.$el, { side: this.tooltipSides })
        .addClass('comments');

        this.renderUntouchedContent(comment);

        this.updateUntouchedTooltip(comment,publishedComments);
    ...
    updateUntouchedTooltip(comment, publishedComments){
        const $list = $('<ul/>');
        const draftComment = this.comment
        const tooltipTemplate = _.template(dedent`
            <li>
             <div class="reviewer">
              <%- user %>:
        ...
        if (draftComment && publishedComments?.length) {
