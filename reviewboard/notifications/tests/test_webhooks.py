...
class WebHookSignalDispatchTests(SpyAgency, TestCase):
    ...
    def test_reply_published(self):
        ...
        # 5 commit message comments
        commit = self.create_diffcommit(diffset=diffset)
        commit_message_comment_1 = self.create_commit_message_comment(
            reply, commit, 
            reply_to=self.create_commit_message_comment(review, commit))
        commit_message_comment_2 = self.create_commit_message_comment(
            reply, commit, 
            reply_to=self.create_commit_message_comment(review, commit))
        ...
        self.assertIn('commit_message_comments', payload)
        comments = payload['commit_message_comments']
        self.assertEqual(len(comments), 2)
        self.assertEqual(comments[0]['id'], commit_message_comment_1.pk)
        self.assertEqual(comments[1]['id'], commit_message_comment_2.pk)
